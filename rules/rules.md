...
- [规则](#%E8%A7%84%E5%88%99)
  ...
  - [箭](#%E7%AE%AD)
    - [箭限制](#%E7%AE%AD%E9%99%90%E5%88%B6)
...
- [1to9填充](rules.md#1to9填充)
- [1toG填充](rules.md#1tog填充)
- [1to8填充](rules.md#1to8填充)
- [1to6填充](rules.md#1to6填充)
- [9选6填充](rules.md#9选6填充)
- [全盘9选6填充](rules.md#全盘9选6填充)
- [1to9不重复](rules.md#1to9不重复)
- [1to8不重复](rules.md#1to8不重复)
- [0to9不重复](rules.md#0to9不重复)
- [摩天楼](rules.md#摩天楼)
...
- [奇偶星](rules.md#奇偶星)
- [骨牌](rules.md#骨牌)
  - [1to9骨牌填充](rules.md#1to9骨牌填充)
- [耳语线](rules.md#耳语线)
- [连续区间线](rules.md#连续区间线)
  - [连续区间限制](rules.md#连续区间限制)
- [熵约束](rules.md#熵约束)
  - [三格熵约束](rules.md#三格熵约束)
  - [2x2熵约束](rules.md#2x2熵约束)
- [熵线](rules.md#熵线)
- [反熵线](rules.md#反熵线)
- [模约束](rules.md#模约束)
- [模线](rules.md#模线)
...
- [杀手框](rules.md#杀手框)
  - [杀手限制](rules.md#杀手限制)
- [职业杀手框](rules.md#职业杀手框)
  - [职业杀手限制](rules.md#职业杀手限制)
- [Psycho look-and-say cages](rules.md#psycho-look-and-say-cages)
- [箭](rules.md#箭)
  - [箭限制](rules.md#箭限制)
- [战争迷雾](rules.md#战争迷雾)
...
![共边平行路](../.gitbook/assets/共边平行区域.png)

### 棱

魔方中的[共边平行路](rules.md#共边平行路)

![棱](../.gitbook/assets/棱.png)

- 棱的编号
  - 方向：`左`，`右`，`下`
  - 数字：从中心点向外，分别为 1,2,3...

![编号棱](../.gitbook/assets/编号棱.png)
...
> 对于 9 阶数独，对角线满足：区域内[1\~9填充](rules.md#1to9填充)
...
![Towers](../.gitbook/assets/Towers.png)
...
- 摩天楼观测提示数 → 从提示数所在（观测位，观测方向）上，可以看到的楼栋数（即[摩天楼观测数](rules.md#摩天楼观测数)）
- 摩天和提示数 → 从提示数所在（观测位，观测方向）上，可以看到的部分楼，[摩天和](rules.md#摩天和)是这些楼的总层数
...
>   - ![盘面](../.gitbook/assets/盘面.png)
> - 观测信息：
>   - 从 `A0`（A1 左侧），向右观测
>   - ![观测位](../.gitbook/assets/观测位.png)
...
>   - ![视角转换](../.gitbook/assets/视角转换.png)
> - 结论：此例中，`A0` 位向右观测的
>   - `摩天楼观测数` 是 `3`
>   - `摩天和` 是 `20`=3+8+9
>   - ![观测数](../.gitbook/assets/观测数.png)
...
如果[共边邻格](rules.md#共边邻格)的公共边上存在标记，说明这两格数字差为 1

### 斜连续

如果[对角邻格](rules.md#对角邻格)的公共顶点上存在标记，说明这两格数字差为 1

## 黑白点

如果[共边邻格](rules.md#共边邻格)的公共边上存在黑白点标记，说明这两格数字大小满足：

- 黑点：两格数字是两格关系
- 白点：两格数字差为 1（即[连续](rules.md#连续)）
...
- 盘面内任意一个数字`A`，它的[邻格](rules.md#邻格)都不是数字 `A`

> 在标准数独中，等价于：
>
> - 盘面内任意一个数字`A`，它的[对角邻格](rules.md#对角邻格)都不是数字 `A`
> - 盘面内任意一个数字`A`，它的[士步格](rules.md#士步)都不是数字 `A`

## 士步

两格的横向距离和纵向距离都是`1`，则互为`士步`格。

> `士步格`等价与 [对角邻格](rules.md#对角邻格)
...
两格互为[马步格](rules.md#马步)，且数字相同

### 马步中心

互为[马步](rules.md#马步)的两格的中点
...
- 区域：中心格 及其[共边邻格](rules.md#共边邻格)，一般标记中心格
- 限制：中心格与其[共边邻格](rules.md#共边邻格)奇偶性相反
...
骨牌 `1`\~`9`如下图所示： ![骨牌](../.gitbook/assets/骨牌.jpeg)
...
- 别名：Renban
- 限制：线上的单元格满足[连续区间限制](rules.md#连续区间限制)
...
- 别名：Entropic
- 限制：线的任何三个连续单元格都满足[熵约束](rules.md#熵约束)

## 反熵线

- 别名：Anti-Entropic
- 限制：线的任何三个连续单元格都不满足[熵约束](rules.md#熵约束)

## 模约束

三格满足`模约束`，当且仅当 三格的数字对 3 取模结果互异。 即恰好包含：
...
- 别名：Modular
- 限制：线的任何三个连续单元格都满足[模约束](rules.md#模约束)
...
杀手框是一种特殊的[计算框](rules.md#计算框)，框内单元格数字各不相同。
...
职业杀手框是满足`暗杀`条件的[杀手框](rules.md#杀手框)
...
Psycho look-and-say cages 是盘面中由"指示圈"框定的若干个单元格。 在"指示圈"左上角存在提示数`CX`。
...
- 指示灯：可驱散当前格和周围[邻格](rules.md#邻格)的迷雾
- 填入数字正确的单元格：可驱散周围[邻格](rules.md#邻格)的迷雾
